const url = "https://image-gallery-backend.herokuapp.com";
/**
 * Returns a list of all items on the server
 */
export async function listGalleryItems() {
  const response = await fetch(`${url}/api/galleryItems`);
...
export async function updateGalleryItem(galleryItem) {
  const response = await fetch(`${url}/api/galleryItems/${galleryItem._id}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function deleteGalleryItem(galleryItem) {
  await fetch(`${url}/api/galleryItems/${galleryItem._id}`, {
    method: "DELETE",
  });
